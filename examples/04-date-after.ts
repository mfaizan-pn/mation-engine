...
const run = async () => {
  ...
  // Step 2: Define the rule input (DATE.AFTER)
  const ruleId = "date-after-last-contact";
  ...
  await engine.compileAndStore(ruleId, ruleInput, {
    rule_name: "Last contacted after timestamp",
  });

  // Step 4: Execute by ID
  const context = {
    company: { last_contacted_at: "2026-03-12T06:30:36.599Z" },
  };
